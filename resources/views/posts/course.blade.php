        ...
        <div id="cardMenu" class="mobile-card-menu mobile-w-100 w-200px">
            ...
            <div class="card {{ (isset($course) && $course->id===$c->id) ? " color-hijau-tua"  : "bg-hijau-tua text-white" }} mb-3">
                <div class="card-body">
                    ...
                    <a href="{{route('course.posts.index',$c)}}" class="stretched-link"></a>
                    <p class="card-text text-end fs-14 mt-2"><small> {{$c->classPosts(Auth::user()->classes->pluck('id'))->count()}} Forum</small></p>
                </div>
            </div>
            @endforeach
        </div>
