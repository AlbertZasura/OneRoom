    ...
    <div class="card-shadow bg-white m-lg-5 border-radius-8px" style="width: auto; height: auto;">
        <div class="card-body p-lg-5">
            ...
            @can('create', App\Models\Message::class)
                <br>
                <button class="btn btn-outline-secondary" id="open-popup"><i class="fas fa-plus"></i></i>&nbsp Tambah
                    Pengumuman</button>
            @endcan
                                ...
                                @can('create', App\Models\Message::class)
                                    <td>
                                        <form action="{{ route('messages.destroy', [$message]) }}"
                                            onsubmit="deleteAnnouncementBtn(event, {{ $message->id }})"
                                            id="deleteAnouncementForm{{ $message->id }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button class="btn" type="submit"><i class="fas fa-times fa-lg"
                                                    style="color:red"></i></button>
                                        </form>
                                    </td>
                                @endcan
                                <td>
                                    <a class="btn btn-fill-green rounded-pill" role="button" id="myButton" type="button"
                                        href="{{ route('messages.show', $message->id) }}">Lihat Detail</a>
                                </td>
                            </tr>
                ...
                <div class="">
                    <div class="">
                        ...
                        <div class="modal-body">
                            <form>
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="title" class="form-control form-input-color" id="title"
                                        value="{{ old('title') }}" required>
                                </div>
                                <div class="mb-3">
                                    <label for="message-text" class="col-form-label"><b>Isi</b></label>
                                    <textarea class="form-control form-input-color" name="content" id="content"
                        ...
                        <div class="d-flex justify-content-end">
                            <a id="action-closes-popup" class="btn btn-outline-green rounded-pill">Batal</a>
                            <button class="btn btn-fill-green rounded-pill" type="submit"
                                id="action-submit-popup">Simpan</button>
                        </div>
                    </div>
                </div>
            </div>
